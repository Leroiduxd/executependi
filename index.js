...
app.post('/run', async (req, res) => {
  ...
  if (!orderId || assetIndex === undefined) {
  ...
  try {
    ...
    if (!data.proof_bytes) {
      return res.status(500).json({ error: 'Invalid proof response from oracle API' });
    }

    const tx = await contract.executePendingOrder(orderId, data.proof_bytes);
    await tx.wait();
